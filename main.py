...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or current_user.id != 1:
            flash("You need to be an administrator to perform that action.")
...
def get_gravatar_url(email, size=100):
    ...
    hash_secret = hashlib.md5(email_encoded).hexdigest()
    return f"https://www.gravatar.com/avatar/{hash_secret}?s={size}&d=identicon"
